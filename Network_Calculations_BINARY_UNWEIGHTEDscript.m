...
ones_zeros = ~eye(240); % Creates an 82x82 matrix of ones with zero across the diagonal.
...
% MEAURES OF FUNCTIONAL SEGREGATION
    ...
    % 3.Modularity 
		...
		for k = 1:g,
			[Ci(:,k), Q(k)]=modularity_finetune_und_sign(CIJ(:,:,k),'sta',Ci0);
		end
...
% MEAURES OF FUNCTIONAL INTEGRATION
 ...
 % MEASURES OF BRAIN ECONOMY
	...
	% 2. Global and Local Efficiency
		...
		% Global Efficiency
			for k = 1:g,
				E_global(:,k) = efficiency_bin(CIJ(:,:,k));
			end

		% Local Efficiency
		local = 1;
			for k = 1:g,
				E_local(:,k) = efficiency_bin(CIJ(:,:,k),local);
			end
	...
	% 3. Rentian Scaling
		 ...
		 %  You want a large enough number to adequately estimate the Rents exponent.
		
		n=5000; 
			for k = 1:g,
				[N E] = rentian_scaling(A,XYZ,n)(W(:,:,k));
		end
		% Reset n to represent number of nodes. 
			n=82;
 ...
 % MEASURES OF CENTRALITY
    ...
    % 1.Betweeness centrality
		...
		for k = 1:g,
			BC = betweenness_bin(CIJ(:,:,k));
        end

	    % 3.Edge betweenness centrality 

        for k = 1:g,
            EBC = edge_betweenness_bin(CIJ(:,:,k));
        end
        
        
    % 2.Within-module degree z-score
        
        % The within-module degree z-score is a within-module version of degree
        % centrality.
    
		for k = 1:g,
            Z(:,k) = module_degree_zscore(CIJ(:,:,k),Ci(:,k));
		end
    ...
    % 3.Participation coefficient
		...
		for k = 1:g,
            P(:,k) = participation_coef(CIJ(:,:,k),Ci(:,k));
		end
 ...
 % MEASURES OF NETWORK RESILIENCE 
    ...
    % 1.Assortativity 
		...
		for k = 1:g,
            r(k) = assortativity_bin(CIJ(:,:,k),flag);
        end 
